...
- [ ] Параллелизация шагов use_cases + nfr (оба зависят только от input)
- [ ] Streaming partial output (показ частичного кода во время генерации)
- [ ] Job history persistence (чтобы переживало рестарт сервера)
- [ ] Per-step model override в UI (не только fast/smart, а каждый шаг отдельно)
